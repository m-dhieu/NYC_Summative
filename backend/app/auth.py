...
# pydantic models representing tokens and users for validation
...
def get_user(db, username: str) -> Optional[UserInDB]:
    """Retrieves a user from DB given username"""
    if username in db:
...
async def get_current_user(token: str = Depends(oauth2_scheme)) -> User:
    """Gets current user from JWT token or raises unauthorized error"""
...
async def get_current_active_user(current_user: User = Security(get_current_user)) -> User:
    """Verifies user is active, not disabled"""
